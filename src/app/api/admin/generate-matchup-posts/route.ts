...
function generateContent(game: {
  ...
  const content = `
<h2>Game Overview</h2>
${pick(INTRO_TEMPLATES, seed)(away, home, week, venue)}
...
${pick(H2H_TEMPLATES, seed + 1)(away, home)}

<h2>Keys to the Game</h2>
${pick(KEYS_TEMPLATES, seed + 2)(away, home, ap, hp)}

<h2>Prediction</h2>
<p><em>Kickoff: ${kickoffStr}${venue ? ` | ${venue}` : ""}${network ? ` | ${network}` : ""}</em></p>
${pick(PREDICTION_TEMPLATES, seed + 3)(away, home, ap, hp)}
`.trim();
...
export async function POST(req: NextRequest) {
  ...
  if (games.length === 0) {
    return NextResponse.json({ message: "No scheduled games found", created: 0, skipped: 0 });
  }
  ...
  return NextResponse.json({
    ...
    created, skipped, total: games.length,
  });
}
...
export async function GET(req: NextRequest) {
  ...
  const withStatus = await Promise.all(
    games.map(async (g) => {
      ...
      return { awayTeam: g.awayTeamFull, homeTeam: g.homeTeamFull, week: g.week, season: g.season, kickoff: g.kickoff, hasPost: !!hasPost };
    })
  );
